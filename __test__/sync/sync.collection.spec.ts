import { Entity, ISyncStorage, SyncCollection } from "../../src/index";
import { uniqueId } from "../../src/utils";
import { SyncTodoStorage } from "../sync_todo_storage";
import { makeTodos, seedTodo, Todo } from "../todo_seeder";
...
describe('SyncCollection', () => {
    ...
    describe('[set]', () => {
        ...
        it('updates an entity if it does exist', async () => {
            ...
            let entityToBeUpdated = (await seedTodo(collection, 1))[0];
    ...
    describe('[get]', () => {
        ...
        it('returns an entity', async () => {
            // Arrange
            const collection = makeCollection();
            let actualEntity = (await seedTodo(collection, 1))[0];
            let expectedEntity;
...
function makeCollection() {
    const storage = new SyncTodoStorage();
    const collection = new SyncCollection<Entity<Todo>>(storage, 'TodayTodos');
    return collection;
}
